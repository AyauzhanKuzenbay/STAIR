...
class ModelProcessor:
    ...
    def _extract_eval_arrays(self, data):
        """
        Expected STAIR test format:
            row[0] = trajectory id (int)
            row[1] = cell-ID sequence, shape (T,) or (T,1)
            row[2] = time-gap sequence Δt   (T,) float  [optional]
            row[3] = displacement sequence Δd (T,) float  [optional]
        """
        ids, trajs, dt_seqs, dd_seqs = [], [], [], []
        has_gaps = False
        ...
        for row in data:
            ...
            try:
                dt = np.asarray(row[2], dtype=np.float32)
                dd = np.asarray(row[3], dtype=np.float32)
                if len(dt) > 0:
                    has_gaps = True
                dt_seqs.append(dt)
                dd_seqs.append(dd)
            except (IndexError, TypeError):
                dt_seqs.append(np.array([], dtype=np.float32))
                dd_seqs.append(np.array([], dtype=np.float32))

        return (np.asarray(ids, dtype=np.int64), trajs,
                dt_seqs if has_gaps else None,
                dd_seqs if has_gaps else None)

    def _encode_in_batches(self, encoder_model, seqs, batch_size,
                            dt_seqs=None, dd_seqs=None):
        """
        Encode trajectories in batches.

        The STAIR encoder expects [ids, delta_t, delta_d] and returns
        [z_norm, H].  If dt_seqs / dd_seqs are None, zero arrays are used
        (backward-compatible with models that ignore gap inputs).
        """
        all_vecs = []
        total = len(seqs)
        use_gap = dt_seqs is not None

        for start in range(0, total, batch_size):
            end = min(start + batch_size, total)
            ids_batch = self._pad_id_sequences(seqs[start:end])   # (B, L)

            if use_gap:
                # Pad Δt and Δd to same length L
                L = ids_batch.shape[1]
                B = ids_batch.shape[0]

                def pad_gap(gap_list, b, l):
                ...
                dt_batch = pad_gap(dt_seqs[start:end], B, L)
                dd_batch = pad_gap(dd_seqs[start:end], B, L)
                pred = encoder_model.predict([ids_batch, dt_batch, dd_batch],
                                             verbose=0)
            else:
                # Fallback: pass zero gap arrays
                B, L = ids_batch.shape
                zeros = np.zeros((B, L, 1), dtype=np.float32)
                pred = encoder_model.predict([ids_batch, zeros, zeros], verbose=0)

            # STAIR encoder returns [z_norm, H]; take z_norm
            if isinstance(pred, (list, tuple)):
                vec = np.asarray(pred[0])  # (B, repr_dim)
            else:
                # Older single-output path: mean-pool hidden states
                if pred.ndim == 3:
    ...
    def model_evaluate(self, encoder_model, arg_processor):
        ...
        gt_ids, gt_trajs, gt_dt, gt_dd = self._extract_eval_arrays(gt_data)
        q_ids, q_trajs, q_dt, q_dd = self._extract_eval_arrays(q_data)
        ...
        gt_vecs = self._encode_in_batches(
            encoder_model, gt_trajs, predict_batch_size, gt_dt, gt_dd)

        print("Encoding query trajectories...")
        q_vecs = self._encode_in_batches(
            encoder_model, q_trajs, predict_batch_size, q_dt, q_dd)
...
# ---------------------------------------------------------------------------
# STAIR-aware encode helper (overrides _encode_in_batches for eval)
# ---------------------------------------------------------------------------
# The STAIR encoder expects three inputs: [ids, delta_t, delta_d].
# During eval we build zero delta_t / delta_d if not provided in data.
# The encoder returns [z_norm, H]; we use z_norm (index 0) directly.
