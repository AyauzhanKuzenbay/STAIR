...
def main():
    ...
    if arg_processor.process_train_val:
        ...
        all_traj = file_reader.read_trajectory_from_file(
            input_file_path,
            dataset_mode,
        ...
        all_train_pairs = traj_processor.first_loop(
            all_train,
            point_drop_rates,
            spatial_distortions,
        ...
        all_validation_pairs = traj_processor.first_loop(
            all_validation,
            point_drop_rates,
            spatial_distortions,
            temporal_distortions,
        ...
        all_train_pairs = traj_processor.second_loop(
            all_train_pairs,
            key_lookup_dict,
            min_trajectory_length,
            min_query_length
        ...
        all_validation_pairs = traj_processor.second_loop(
        ...
        train_data = traj_processor.process_training_data(
            train_data, output_directory=output_directory)

        # Pass training stats to validation — ensures identical normalisation
        train_stats = traj_processor.get_interval_stats()

        print("Preparing validation data")
        val_data = traj_processor.flatten_traj_pairs(all_validation_pairs)
        traj_processor.set_interval_stats(train_stats)
    ...
    if arg_processor.process_test:
        ...
        if not arg_processor.process_train_val:
            ...
            cell_dict_name = arg_processor.cell_dict_name
            key_lookup_dict = file_reader.read_npy(
                output_directory,
                cell_dict_name
            ).item()
        ...
        # If we did NOT generate train/val in this run, start test from the beginning
        if not arg_processor.process_train_val:
            line_start = 0
        else:
        ...
        dataset_mode = arg_processor.dataset_mode
        import os as _os
        stats_path = _os.path.join(output_directory, 'interval_stats.npy')
        test_processor = TestFileProcessor(
            input_file_path,
            line_start,
            bbox_coords,
            all_grids,
        ...
        [q, qdb, qraw, qdbraw] = test_processor.process_data(
            num_q,
            dataset_mode,
            min_traj_len,
            max_traj_len,
        ...
        # Check if we got any valid trajectories
        if len(q) == 0:
            print("\n" + "=" * 70)
            print("ERROR: No valid trajectories found in the test data!")
            print("=" * 70)
            print(f"\nCurrent settings:")
            print(f"  - Starting line: {line_start}")
            print(f"  - File has only ~2,838 lines total")
            print(f"  - Minimum trajectory length (after split): {min_traj_len}")
            print(f"  - Maximum trajectory length: {max_traj_len}")
            print(f"  - Dataset mode: {dataset_mode}")
            print(f"\n❌ PROBLEM: Starting at line {line_start} is past the end of your file!")
            print(f"\n✅ SOLUTION: Update your .ini config file with:")
            print(f"   - Set line_start = 0 (or remove training data processing)")
            print(f"   - Set min_trajectory_length = 20 (currently too strict)")
            print(f"   - Or use a larger dataset file")
            test_processor.close_file()
            sys.exit(1)

        # Start the ID from the query's biggest ID
        db_start_ID = q[-1][0] + 1
        [db, _, dbraw, _] = test_processor.process_data(
            num_db,
            dataset_mode,
            min_traj_len,
            max_traj_len,
